...
function apriori(D, minSup, minConf) {
  ...
  const [allRules, filteredRules] = generateAssociationRules(
    L,
    allCounts,
    minConf
  );
...
function findFrequentOneItemsets(D, minSup) {
  ...
  const firstItemSet = Object.keys(counts).filter(
    (item) => counts[item] >= minSup
  );
...
function getSubsets(itemset, k) {
  ...
  const subsetsWithHead = getSubsets(tail, k - 1).map((subset) => [
    head,
    ...subset,
  ]);
...
function generateAssociationRules(L, allCounts, minConf) {
  ...
  const lastNonEmptyIndex = L.reduce(
    (acc, cur, idx) => (cur.length > 0 ? idx : acc),
    -1
  );
    ...
    for (let r = 1; r < itemset.length; r++) {
      ...
      for (let antecedent of allNonEmptySubsets) {
        ...
        const antecedentCount =
          allCounts[antecedent.length - 1][antecedent.join(",")];
